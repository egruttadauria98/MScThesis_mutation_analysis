...
class DataHelper:
    ...
    def gen_full_alignment(self):
        ...
        def compute_sequence_weights(self):
            X_flat = self.x_train.reshape((self.x_train.shape[0], self.x_train.shape[1]*self.x_train.shape[2]))
            N_list = torch.empty(X_flat.shape[0], X_flat.shape[1])
            
            for i, seq in enumerate(X_flat):
                N_list[i, :] = 1.0 / torch.sum(torch.dot(X_flat, seq) / torch.dot(seq, seq) > 1 - self.theta)

            self.weights = N_list[0]

            
        # Fast sequence weights with Theano
        if self.calc_weights:
            print ("Computing sequence weights")
            self.compute_sequence_weights()
